...
def main():
    ...
    parser.add_argument('split', help='split configs in BboxToolkit/tools/split_configs')
    ...
    for i, img_name in enumerate(img_list):
        ...
        # test a single image
        result = inference_detector_huge_image(model, img_name, args.split, nms_cfg)

        print('Draw image results!!')
        # The following code is taken from: mmdet/models/detectors/obb/obb_base.py
        ...
        if segm_result is not None and len(labels) > 0:  # non empty
            ...
            np.random.seed(42)
            color_masks = [
                np.random.randint(0, 256, (1, 3), dtype=np.uint8) for _ in range(max(labels) + 1)
            ]
            for i in inds:
                i = int(i)
        ...
        if args.method == 'pyplot':
            ...
            for bbox, label, score in zip(bboxes, labels, scores):
                img = cv2.drawContours(img, [np.int64(np.array_split(bbox, 4, axis=0))],
                                       -1, (255, 0, 255),
                                       thickness=6)
                text = f'{model.CLASSES[label]}|{score:.2f}'
                img = cv2.putText(img,
                                  text, (int(min(bbox[0::2])), int(min(bbox[1::2]))),
